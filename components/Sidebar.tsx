...
export default function Sidebar() {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      href: "/pomodoro",
      gradient:
        "radial-gradient(circle, rgba(251, 146, 60, 0.15) 0%, rgba(245, 158, 11, 0.06) 50%, rgba(217, 119, 6, 0) 100%)",
    ...
    },
    {
      ...
      href: "/journal",
      gradient:
        "radial-gradient(circle, rgba(139, 92, 246, 0.15) 0%, rgba(124, 58, 237, 0.06) 50%, rgba(103, 23, 205, 0) 100%)",
  ...
  const profileItem: MenuItem = {
    ...
    href: "/profile",
    gradient:
      "radial-gradient(circle, rgba(129, 140, 248, 0.15) 0%, rgba(99, 102, 241, 0.06) 50%, rgba(79, 70, 229, 0) 100%)",
    iconColor: "text-indigo-500",
    hoverColor: "group-hover:text-indigo-500",
  }

  const icon = !hasMounted ? null : resolvedTheme === "dark" ? (
  ...
  const handleLinkClick = (href: string, itemOnClick?: () => void) => {
  ...
  const renderNavItem = (item: MenuItem) => (
    <motion.li key={item.label} className={cn("relative w-full flex justify-center list-none", isExpanded && "px-3")}>
      <motion.div
        className="block rounded-xl group relative w-full"
  ...
  return (
    ...
    >
      <motion.div
        className={
          "absolute -inset-2 bg-gradient-radial from-transparent via-blue-400/30 via-30% via-purple-400/30 via-60% via-red-400/30 via-90% to-transparent rounded-3xl z-0 pointer-events-none"
        }
      ...
      <nav className="flex flex-col w-full items-center gap-2 relative z-10">{menuItems.map(renderNavItem)}</nav>
